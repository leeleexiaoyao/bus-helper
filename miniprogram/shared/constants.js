...
function buildSeedSeatCodes(templateId) {
    ...
    return template.rowSeatCounts.flatMap((seatCount, rowIndex) => SEAT_LETTERS.slice(0, seatCount).map((letter) => `${rowIndex + 1}${letter}`));
}
function createSeedSeatMap(seatCodes) {
    return seatCodes.reduce((accumulator, seatCode) => {
        accumulator[seatCode] = null;
        return accumulator;
    }, {});
}
function buildSeedPassengerUsers(tripId) {
    return Array.from({ length: SEED_OCCUPIED_USER_COUNT - exports.DEMO_USERS.length }, (_, index) => {
        const userNumber = index + exports.DEMO_USERS.length + 1;
        return {
            id: `user-${userNumber}`,
...
function buildSeedTripMembers(users, tripId, joinedAt) {
...
function buildSeedTrip(users, seatCodes) {
...
function createSeededDemoAppState() {
    ...
    const switchableUsers = exports.DEMO_USERS.map((user) => (Object.assign(Object.assign({}, user), { currentTripId: SEED_TRIP_ID, isAuthorized: true })));
    ...
    const seededTripMembers = buildSeedTripMembers(seededUsers.slice(0, SEED_OCCUPIED_USER_COUNT), SEED_TRIP_ID, seededTrip.createdAt);
    return {
        version: exports.APP_STATE_VERSION,
        users: seededUsers.reduce((accumulator, user) => {
            accumulator[user.id] = Object.assign({}, user);
            return accumulator;
        }, {}),
        trips: {
